...
TEXT ·p256PointAddAsm(SB),0,$392-80
	...
	// Begin point add
	LDx(z2in)
	CALL	btcSqrInternal<>(SB)    // z2²
	STy(z2sqr)

	CALL	btcMulInternal<>(SB)    // z2³

	LDx(y1in)
	CALL	btcMulInternal<>(SB)    // s1 = z2³*y1
	STy(s1)

	LDx(z1in)
	CALL	btcSqrInternal<>(SB)    // z1²
	STy(z1sqr)

	CALL	btcMulInternal<>(SB)    // z1³

	LDx(y2in)
	CALL	btcMulInternal<>(SB)    // s2 = z1³*y2
	...
	LDx(z2sqr)
	LDy(x1in)
	CALL	btcMulInternal<>(SB)    // u1 = x1 * z2²
	STy(u1)

	LDx(z1sqr)
	LDy(x2in)
	CALL	btcMulInternal<>(SB)    // u2 = x2 * z1²
	STy(u2)
	...
	LDx(r)
	CALL	btcSqrInternal<>(SB)    // rsqr = r²
	STy(rsqr)

	LDx(h)
	CALL	btcSqrInternal<>(SB)    // hsqr = h²
	STy(hsqr)

	LDx(h)
	CALL	btcMulInternal<>(SB)    // hcub = h³
	STy(hcub)
	...
	LDx(hsqr)
	LDy(u1)
	CALL	btcMulInternal<>(SB)    // h² * u1
	STy(u2)

	p256MulBy2Inline               // u1 * h² * 2, inline
	LDy(rsqr)
	CALL	btcSubInternal<>(SB)    // r² - u1 * h² * 2
