...
func Ecrecover(hash, sig []byte) ([]byte, error) {
	...
	px, py, err := btc.BTCasm().Recover(r, s, msg, v)
	if err != nil {
		return nil, err
	}
	return btc.Marshal(S256(), px, py), nil
}
...
func Sign(digestHash []byte, prv *ecdsa.PrivateKey) (sig []byte, err error) {
	...
	r, s, v, err := btc.BTCasm().Sign(rand.Reader, msg, priv)
	if err != nil {
		return nil, err
	}
	sig = make([]byte, 65)
...
func VerifySignature(pubkey, digestHash, signature []byte) bool {
	...
	px, py := btc.Unmarshal(S256(), pubkey)
	if px == nil || py == nil {
		return false
	}
	...
	msg := new(big.Int).SetBytes(digestHash)
	return btc.BTCasm().Verify(r, s, msg, px, py)
}
