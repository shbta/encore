...
func TestKeyStoreSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)

	a, err := ks.NewAccountSM2("foo")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignWithPassphraseSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)

	pass := "passwd"
	acc, err := ks.NewAccountSM2(pass)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTimedUnlockSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)
...
func TestOverrideUnlockSM2(t *testing.T) {
...
func TestSignRaceSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, false)
	defer os.RemoveAll(dir)

	// Create a test account.
	a1, err := ks.NewAccountSM2("")
	if err != nil {
...
func TestWalletNotificationsSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, false)
	defer os.RemoveAll(dir)
...
func TestImportSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)
	key, err := sm2crypto.GenerateKey()
	if err != nil {
		t.Fatalf("failed to generate key: %v", key)
	}
	if _, err = ks.ImportSM2(key, "old"); err != nil {
		t.Errorf("importing failed: %v", err)
	}
	if _, err = ks.ImportSM2(key, "old"); err == nil {
		t.Errorf("importing same key twice succeeded")
	}
	if _, err = ks.ImportSM2(key, "new"); err == nil {
		t.Errorf("importing same key twice succeeded")
	}
}

// TestImportSM2 tests the import and export functionality of a keystore.
func TestImportExportSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)
	acc, err := ks.NewAccountSM2("old")
	if err != nil {
...
func TestImportRaceSM2(t *testing.T) {
	dir, ks := tmpKeyStore(t, true)
	defer os.RemoveAll(dir)
	acc, err := ks.NewAccountSM2("old")
	if err != nil {
